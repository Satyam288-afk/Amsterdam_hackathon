...
export default function RecoveryCasePage() {
  ...
  const [callStage, setCallStage] = useState<"dialling" | "connected">("dialling");
  const load = () => id && recoveryDemo.getCase(id).then(setItem);
  useEffect(() => { load(); }, [id]);
  useEffect(() => {
    if (!callOpen) return;
    setCallStage("dialling");
  ...
  return <div className="space-y-6">
    ...
    {callOpen && <CallSimulator caseItem={item} stage={callStage} busy={busy} onClose={() => setCallOpen(false)} onOutcome={completeCall} />}
  </div>;
}

function CallSimulator({ caseItem, stage, busy, onClose, onOutcome }: { caseItem: RecoveryCase; stage: "dialling" | "connected"; busy: boolean; onClose: () => void; onOutcome: (response: DemoResponse) => void }) {
  const connected = stage === "connected";
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 p-4"><div className="w-full max-w-lg rounded-2xl bg-[#fffaf0] p-6 shadow-2xl"><div className="flex items-start justify-between"><div><p className="text-xs font-black tracking-wider text-[#8b572f]">SIMULATED VOICE CALL · NO REAL NUMBER DIALED</p><h2 className="mt-1 text-2xl font-black text-[#2d1e18] font-display">Sambhaash Recovery Agent</h2></div><button onClick={onClose} className="rounded-full p-2 text-[#6e4627] cursor-pointer"><X /></button></div><div className="mt-6 rounded-xl bg-[#2d1e18] p-5 text-[#fffaf0]"><div className="flex items-center gap-3"><span className={`h-3 w-3 rounded-full ${connected ? "bg-emerald-400" : "animate-pulse bg-amber-300"}`} /><div><p className="font-bold">{connected ? `Connected to ${caseItem.customer_name}` : "Dialling recovery call…"}</p><p className="text-sm text-white/65">Hindi / Hinglish · Invoice {caseItem.invoice_number}</p></div></div>{connected && <div className="mt-5 space-y-3 text-sm"><p><strong>Agent:</strong> Namaste, {caseItem.customer_name}. Aapka ₹{caseItem.amount.toLocaleString("en-IN")} invoice overdue hai. Kya main secure payment link share kar doon?</p><p className="rounded-lg bg-white/10 p-3 text-white/80"><strong>Live transcript:</strong> Choose a customer response below. The same policy engine will record the outcome and update the audit trail.</p></div>}</div>{connected && <div className="mt-5 grid gap-2"><button disabled={busy} onClick={() => onOutcome("PROMISE_TO_PAY")} className="rounded-lg bg-[#d4a373] px-4 py-3 text-left font-bold text-white cursor-pointer">“Friday ko payment kar denge” <span className="block text-xs font-normal text-white/80">Record promise and stop outreach</span></button><button disabled={busy} onClick={() => onOutcome("PAYMENT_CONFIRMED")} className="rounded-lg border border-emerald-300 bg-emerald-50 px-4 py-3 text-left font-bold text-emerald-900 cursor-pointer">“Payment complete ho gaya” <span className="block text-xs font-normal text-emerald-800">Confirm recovery and close the case</span></button><button disabled={busy} onClick={() => onOutcome("DISPUTE")} className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-left font-bold text-amber-900 cursor-pointer">“Invoice amount disputed hai” <span className="block text-xs font-normal text-amber-800">Escalate to a human reviewer</span></button></div>}</div></div>;
}
